...
const camera = { available: true, bridge_registered: true, go2rtc_reachable: true, online: true };
...
describe("relay API contracts", () => {
  ...
  it("accepts the production daily camera projection without diagnostic consumer counts", () => {
    expect(parseHomeResponse({ devices: [{ ...device, camera }], status: {} }).devices[0]?.camera).toEqual(camera);
  });
