  ...
  return <ScheduleDialog closeDisabled={pending} describedBy="schedule-editor-description" labelledBy="schedule-editor-title" onClose={onClose} triggerRef={triggerRef}>
    <form aria-describedby="schedule-editor-description" onSubmit={submit}>
      ...
      <div className="form-grid">
        <label className="field" htmlFor="schedule-time">Time<input aria-describedby={form.formState.errors.executionTime ? "schedule-time-error" : undefined} aria-invalid={Boolean(form.formState.errors.executionTime)} id="schedule-time" type="time" {...form.register("executionTime", { pattern: { value: /^(?:[01]\d|2[0-3]):[0-5]\d$/, message: "Enter a valid time." }, required: "Time is required." })} />{form.formState.errors.executionTime && <span className="form-error" id="schedule-time-error">{form.formState.errors.executionTime.message}</span>}</label>
        <label className="field" htmlFor="schedule-portions">Portions<input aria-describedby={form.formState.errors.grainNum ? "schedule-portions-error" : undefined} aria-invalid={Boolean(form.formState.errors.grainNum)} id="schedule-portions" max={48} min={1} type="number" {...form.register("grainNum", { max: { value: 48, message: "Maximum is 48 portions." }, min: { value: 1, message: "Minimum is 1 portion." }, required: true, valueAsNumber: true })} />{form.formState.errors.grainNum && <span className="form-error" id="schedule-portions-error">{form.formState.errors.grainNum.message}</span>}</label>
      </div>
      ...
      <label className="field" htmlFor="schedule-audio-times">Sound repetitions<input aria-describedby={form.formState.errors.audioTimes ? "schedule-audio-times-error" : undefined} aria-invalid={Boolean(form.formState.errors.audioTimes)} disabled={!form.watch("enableAudio")} id="schedule-audio-times" max={5} min={1} type="number" {...form.register("audioTimes", { max: { value: 5, message: "Maximum is 5 repetitions." }, min: { value: 1, message: "Minimum is 1 repetition." }, valueAsNumber: true })} />{form.formState.errors.audioTimes && <span className="form-error" id="schedule-audio-times-error">{form.formState.errors.audioTimes.message}</span>}</label>
